...
def has_permission(permission):
    def wrap(f):
        def wrapped_f(user, post, *args):
            permission = "%s:%s" % (permission, post.id)
            print permission
            f(user, post, *args)
        return wrapped_f
    return wrap
...
# def has_permission(permission):
#     def call_permission(function):
#         print "has: "+permissions
#         def internal(user, post, *args):
#             permission = "%s:%s" % (permission, post.id)
#             try:
#                 user.permissions[permission]
#                 function(user, *args)
#             except:
#                 print ">> Erro: "+user.name+" não tem permissão de editar post"
#         return internal

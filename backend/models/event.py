...
class Event(ndb.Model):
    ...
    # The start month of the event
    start_month = ndb.IntegerProperty(required=True)

    # The end month of the event
    end_month = ndb.IntegerProperty(required=True)

    # The start year of the event
    start_year = ndb.IntegerProperty(required=True)

    # The end year of the event
    end_year = ndb.IntegerProperty(required=True)
    ...
    def create(data, author, institution):
        ...
        event.start_month = event.start_time.month
        event.start_year = event.start_time.year
        event.end_time = datetime.strptime(
            data.get('end_time'), "%Y-%m-%dT%H:%M:%S")
        event.end_month = event.end_time.month
        event.end_year = event.end_time.year
